...
镜像内置 WebUI、Nginx 与 API 代理配置，无需手动配置。浏览器访问 `http://<主机>:8080`，登录你的 qBittorrent 账号即可使用。

> `QBITTORRENT_URL` 是 qBittorrent WebUI 的地址。默认值 `http://host.docker.internal:8080` 适用于 Docker Desktop；Linux 下可改为宿主机 IP，例如 `http://192.168.1.100:8080`。
